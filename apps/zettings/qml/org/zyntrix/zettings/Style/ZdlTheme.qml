...
Item {
    ...
    readonly property color base50: "#fafafa"
    readonly property color base100: "#f4f4f5"
    readonly property color base200: "#e4e4e7"
    readonly property color base300: "#d4d4d8"
    readonly property color base400: "#a1a1aa"
    readonly property color base500: "#71717a"
    readonly property color base600: "#52525b"
    readonly property color base700: "#3f3f46"
    readonly property color base800: "#27272a"
    readonly property color base900: "#18181b"
    readonly property color base950: "#09090b"
    ...
    readonly property color surface: {
        if (themeMode === ZdlTheme.ThemeMode.Light)
            return base50;
        if (themeMode === ZdlTheme.ThemeMode.Dark)
            return base900;
        if (themeMode === ZdlTheme.ThemeMode.Oled)
            return "#000000";
        return "#000000";
    }
    ...
    readonly property color surfaceMuted: {
        if (themeMode === ZdlTheme.ThemeMode.Light)
            return base100;
        if (themeMode === ZdlTheme.ThemeMode.Dark)
            return Qt.rgba(base800.r, base800.g, base800.b, 0.6);
        if (themeMode === ZdlTheme.ThemeMode.Oled)
            return "#0a0a0a";
        return "#000000";
    }
    ...
    readonly property color shadowTint: {
        ...
        if (themeMode === ZdlTheme.ThemeMode.Dark)
            return Qt.rgba(0, 0, 0, 0.40);
        if (themeMode === ZdlTheme.ThemeMode.Oled)
            return Qt.rgba(0, 0, 0, 0.60);
        return Qt.rgba(0, 0, 0, 0.60);
    }

    readonly property color warning: darkAppearance ? warningDark : warningLight
    readonly property color danger: darkAppearance ? dangerDark : dangerLight
    ...
    readonly property int rowMinHeight: compactDensity ? densityCompactRowHeight : densityComfortableRowHeight
    ...
    FrameAnimation {
        ...
        onTriggered: {
            ...
            const dtMs = (frameTicker.frameTime - frameTicker.previousFrameTime) * 1000;
            if (dtMs <= 0 || dtMs > 500)
                return;
            ...
            if (ZdlTheme.debugFrames && ZdlTheme._windowSamples >= 300) {
                console.info("frame stats:", "avg", ZdlTheme.frameAverageMs.toFixed(2), "ms", "worst(300f)", ZdlTheme.frameWorstMs.toFixed(2), "ms", "budget", ZdlTheme.frameBudgetMs, "ms");
